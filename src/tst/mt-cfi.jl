#=
Notes:
This is a final calculation of this cross-validation.
The overall project is to be concluded in Sept. 8.
I need to analysis one extra trait, and then write the paper.
I was so worn out before the summer holiday.
There are still 24hrs needed to finish my other program(2021-08-19).
I need to collect myself to finish this.
...
3. add_cfi()
=#
using DataFrames, JLD2, CSV, LinearAlgebra, SparseArrays, Statistics, CodecZlib
import RD2:dat_dir
...
function cv_21_08_23()
    @load "$dat_dir/jld/cv-setup.jld" dcs gcs ncs
    @load "$dat_dir/jld/drp-training-2021-08-23.jld" dts gts nts
    @load "$dat_dir/jld/ebv-2021-08-23.jld"  dbv gbv nbv
    ...
    # The trait, CFI, below was calculated on Aug. 23, 2021
    begin                       # The trait of calving to 1st insemination
        dt, dv = make_tv(dts, dcs, dbv, :CFI, :CFI_ERC)
        gt, gv = make_tv(gts, gcs, gbv, :CFI, :CFI_ERC)
        nt, nv = make_tv(nts, ncs, nbv, :CFI, :CFI_ERC)
        rst = mk_cv_tbl(dt, dv, gt, gv, nt, nv, G, :CFI, :CFI_ERC)
        for row in eachrow(rst)
            println("| ", join(row, " | "), " |")
        end
    end
...
"""
I made some mistake in the `add_cfi` function.
So I take the `cfi` part separately.
the function above should be enough later.
"""
function run_cfi()
    @load "$dat_dir/jld/cv-setup.jld" dcs gcs ncs
    @load "$dat_dir/jld/drp-training-2021-08-23.jld" dts gts nts
...
function add_cfi()
    @info "A fifth trait, calving to 1st insemination (CFI), is to be added"
    @info "Rename corresponding columns in the training set"
    @load "$dat_dir/jld/drp-training-2021-04-23.jld" dts gts nts
    rename!(dts, :x12 => :CFI, :x13 => :CFI_ERC)
    rename!(gts, :x12 => :CFI, :x13 => :CFI_ERC)
    rename!(nts, :x14 => :CFI, :x15 => :CFI_ERC)
    @save joinpath(dat_dir, "jld/drp-training-2021-08-23.jld") {compress=true} dts gts nts

    @load "$dat_dir/jld/ebv_202105.jld"  dbv gbv nbv
    rename!(dbv, :x12 => :CFI, :x13 => :CFI_ERC)
    rename!(gbv, :x12 => :CFI, :x13 => :CFI_ERC)
    ...
    df1 = select(df, :ID, :r_Idx_CFI1_4 => :CFI)
    df1[df1.CFI .== ".", :CFI] .= "999"
    df1.CFI = parse.(Float64, df1.CFI)
    nbv = innerjoin(nbv, df1, on=:ID)
    @save joinpath(dat_dir, "jld/ebv-2021-08-23.jld") {compress=true} dbv gbv nbv
end
